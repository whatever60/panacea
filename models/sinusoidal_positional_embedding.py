...
class SinusoidalPositionalEmbedding(nn.Module):
    ...
    @staticmethod
    def get_embedding(counts, embedding_dim, padding_idx=None):
        """Build sinusoidal embeddings.
        This matches the implementation in tensor2tensor, but differs slightly
        from the description in Section 3.5 of "Attention Is All You Need".
        """
        length = counts.size(1)
        half_dim = embedding_dim // 2
        emb = math.log(10000) / (half_dim - 1)
        emb = torch.exp(torch.arange(half_dim, dtype=torch.float) * -emb)
        # emb = torch.arange(num_embeddings, dtype=torch.float).unsqueeze(1) * emb.unsqueeze(0)
        emb = counts.float().unsqueeze(1) * emb.unsqueeze(0)
    ...
    def forward(self, counts, incremental_state=None, timestep=None, **kwargs):
        """Input is expected to be of size [bsz x seqlen]."""
        # bsz, seq_len = torch.onnx.operators.shape_as_tensor(input)
        bsz, seq_len = counts.size()
